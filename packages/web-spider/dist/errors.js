...
export class FetchTransportError extends Error {
    constructor(options) {
        ...
        this.code = "fetch-transport-failed";
...
function classifyKnownFailure(error) {
    const chain = causeChain(error);
    const codes = chain.map(errorCode).filter((code) => code !== undefined);
    const names = chain.map(errorName).filter((name) => name !== undefined);
    ...
    if (codes.some((code) => TLS_CODES.has(code) || code.startsWith("ERR_TLS_") || code.startsWith("ERR_SSL_")) ||
...
export function isLikelyFetchTransportFailure(error) {
 ...
 */
export function toFetchTransportError(error, options = {}) {
    if (error instanceof FetchTransportError)
        return error;
    const descriptor = options.timedOut
        ? { kind: "timeout", diagnostic: "Request timed out", retryable: true }
