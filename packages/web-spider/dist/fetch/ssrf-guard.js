...
const BLOCKED_IPV4_ADDRESSES = ["255.255.255.255"];
const BLOCKED_IPV6_SUBNETS = [
    ["fc00::", 7],
    ["fe80::", 10],
];
const BLOCKED_IPV6_ADDRESSES = ["::1"];
function buildBlockList() {
    ...
    for (const address of BLOCKED_IPV6_ADDRESSES)
        list.addAddress(address, "ipv6");
    return list;
}
...
export function isBlockedAddress(address, allowList) {
    const family = isIP(address);
    if (family === 0)
        return false; // not a literal IP -- caller resolves first
    ...
    if (!blocked)
        return false;
    if (allowList?.check(address, familyName))
        return false;
    if (mapped !== undefined && allowList?.check(mapped, "ipv4"))
        return false;
    return true;
}
function parseAllowRanges(ranges) {
    ...
    for (const range of ranges) {
        ...
        const family = isIP(address);
        if (family === 0)
            throw new Error(`Invalid allowRanges entry (not an IP/CIDR): "${range}"`);
...
export class DefaultSsrfGuard {
    ...
    async assertAllowed(url) {
        ...
        if (isIP(hostname) !== 0) {
            if (isBlockedAddress(hostname, this.allowList))
                throw new SsrfBlockedError(hostname, hostname);
            return;
        }
        const records = await dnsLookup(hostname, { all: true, verbatim: true });
        for (const { address } of records) {
            if (isBlockedAddress(address, this.allowList))
...
export function createSsrfGuard(options) {
