...
describe("spider() — preferMediaWiki", () => {
		...
		const httpClient = stubClient({
			...
			"https://en.wikipedia.org/w/api.php?action=parse&page=Example&prop=text&format=json&redirects=1": {
				parse: {
					title: "Example",
					text: {
						"*": "<p>Real article content, long enough for Readability's heuristics to treat this as a genuine article body rather than boilerplate noise.</p>",
					},
				},
			},
		});
		...
		const httpClient = stubClient(
			{},
			{
				"https://en.wikipedia.org/":
					"<html><head><title>Wikipedia Home</title></head><body><article><p>Real homepage content, long enough for Readability's heuristics to treat this as a genuine article body.</p></article></body></html>",
			},
		);
	...
	it("falls through to the normal fetch path unchanged when the site isn't MediaWiki-based at all", async () => {
		const httpClient = stubClient(
			{},
			{
				"https://example.com/wiki/Something":
					"<html><head><title>Not A Wiki</title></head><body><article><p>Real content here, long enough for Readability's extraction heuristics to actually kick in.</p></article></body></html>",
			},
		);
