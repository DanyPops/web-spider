...
describe("parseGitHubUrl", () => {
	...
	it("parses an issue URL", () => {
		expect(parseGitHubUrl("https://github.com/facebook/react/issues/1")).toEqual({
			owner: "facebook",
			repo: "react",
			kind: "issue",
			number: 1,
	...
	it("parses a pull request URL (same shape as issues -- GitHub's own API treats them the same way)", () => {
		expect(parseGitHubUrl("https://github.com/facebook/react/pull/1")).toEqual({
			owner: "facebook",
			repo: "react",
			kind: "issue",
			number: 1,
	...
	it("builds a structured summary + README markdown", async () => {
		const httpClient = apiClient({
			"https://api.github.com/repos/DanyPops/web-spider": {
				...
				description: "AI-agent-friendly web spider",
				stargazers_count: 5,
				language: "TypeScript",
				topics: ["ai", "crawler"],
				default_branch: "main",
				html_url: "https://github.com/DanyPops/web-spider",
			},
