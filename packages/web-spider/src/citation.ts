 ...
 */
function stripSnippetTruncation(snippet: string): string {
 ...
 */
export function buildTextFragmentUrl(url: string, snippetText: string): string | undefined {
	...
	if (hashIndex === -1) return `${url}#:~:${fragment}`;
