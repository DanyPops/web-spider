...
export function pageToRecords(page: SpideredPage): NDJSONRecord[] {
...
export function pagesToNDJSON(pages: SpideredPage[]): string {
...
export async function ingestToScribe(pages: SpideredPage[], ingestURL: string): Promise<void> {
...
function buildSections(page: SpideredPage): Array<{ name: string; text: string }> {
	const sections: Array<{ name: string; text: string }> = [];
	if (page.description) sections.push({ name: "description", text: page.description });
	if (page.markdown) sections.push({ name: "content", text: truncate(page.markdown, 4000) });
...
function chunkToNode(page: SpideredPage, chunk: Chunk): NDJSONRecord {
	return {
		type: "node",
		id: chunk.id,
