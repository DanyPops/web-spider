...
export function createSessionActDetails(input: {
	...
	switch (input.action) {
		case "navigate":
			return { ...base, summary: "Navigated", items: [], total: 0, truncated: false };
		case "click":
			return { ...base, summary: "Clicked", items: [], total: 0, truncated: false };
		case "hover":
			return { ...base, summary: "Hovered", items: [], total: 0, truncated: false };
		case "pressKey":
			return { ...base, summary: "Key pressed", items: [], total: 0, truncated: false };
		case "type":
			return { ...base, summary: "Typed", items: [], total: 0, truncated: false };
		case "select":
			return { ...base, summary: "Option selected", items: [], total: 0, truncated: false };
		case "waitFor":
			return { ...base, summary: "Condition met", items: [], total: 0, truncated: false };
		case "handleDialog":
			return { ...base, summary: "Dialog policy armed", items: [], total: 0, truncated: false };
		...
		case "downloads": {
			const rows = Array.isArray(input.result)
				? input.result.map(
						(d) => bounded(typeof d === "object" && d && "filename" in d ? String((d as { filename: unknown }).filename) : String(d)) ?? "",
					)
				: [];
			const { items, total, truncated } = boundedItems(rows);
		...
		case "tabs": {
			...
			if (input.result && typeof input.result === "object" && "closedIndex" in input.result) {
				...
				return {
					...base,
					summary: `Tab ${r.closedIndex} closed${r.newActiveIndex !== null ? ` · tab ${r.newActiveIndex} active` : ""}`,
					items: [],
					total: 0,
					truncated: false,
				};
		...
		case "snapshot": {
			...
			return {
				...base,
				summary: `Accessibility snapshot · ${lineCount} line${lineCount === 1 ? "" : "s"}`,
				items: [],
				total: 0,
				truncated: false,
		...
		case "screenshot":
			return { ...base, summary: "Screenshot captured", items: [], total: 0, truncated: false, hasImage: true };
		case "eval": {
			let pretty: string;
			try {
				pretty = JSON.stringify(input.result, null, 2) ?? "undefined";
			} catch {
				pretty = String(input.result);
			}
...
export function parseSessionDetails(value: unknown): SessionPresentationDetails | undefined {
	try {
		...
		if (details.body !== undefined && (typeof details.body !== "string" || details.body.length > DETAILS_MAX_BODY_CHARACTERS + 64))
			return undefined;
...
export function renderWebSessionCall(
	args: Record<string, unknown>,
	theme: Theme,
	context: SessionCallContext = { lastComponent: undefined },
): Text {
...
export class SessionResultCard implements Component {
	...
	render(width: number): string[] {
		...
		const identity = [details.name, details.snapshotVersion !== undefined ? `v${details.snapshotVersion}` : undefined]
			.filter(Boolean)
			.join(" · ");
		const header = [theme.fg("success", `✓ ${details.summary}`), identity ? theme.fg("dim", identity) : undefined]
			.filter(Boolean)
			.join("  ");
		...
		if (expanded && details.body) {
			...
			const capped =
				bodyLines.length > EXPANDED_PRIMARY_MAX_LINES
					? [
							...bodyLines.slice(0, EXPANDED_PRIMARY_MAX_LINES),
							theme.fg("warning", `… ${bodyLines.length - EXPANDED_PRIMARY_MAX_LINES} lines omitted`),
						]
					: bodyLines;
			lines.push(...capped);
