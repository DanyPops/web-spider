...
describe("web_session — the seven newer actions, end to end through the tool boundary", () => {
	it("hover reveals a CSS :hover-only element, and pressKey submits via Enter scoped to a selector", async () => {
		...
		await h.invokeTool("web_session", {
			operation: "act",
			name: "interact-session",
			snapshotVersion: 0,
			action: "navigate",
			url: `${server.baseUrl}/interactive`,
		});
		await h.invokeTool("web_session", {
			operation: "act",
			name: "interact-session",
	...
	it("consoleMessages and networkRequests surface real captured session activity", async () => {
		...
		await h.invokeTool("web_session", {
			operation: "act",
			name: "observability-tool-session",
			snapshotVersion: 0,
			action: "navigate",
			url: `${server.baseUrl}/interactive`,
		});
...
describe("web_session — validation and error propagation", () => {
	it("requires name for create", async () => {
		await expect(h.invokeTool("web_session", { operation: "create" })).rejects.toThrow("name is required for operation=create");
	});

	it("requires name for act", async () => {
		await expect(h.invokeTool("web_session", { operation: "act", snapshotVersion: 0, action: "click" })).rejects.toThrow(
			"name is required for operation=act",
		);
	});

	it("requires snapshotVersion for act", async () => {
		await expect(h.invokeTool("web_session", { operation: "act", name: "x", action: "click" })).rejects.toThrow(
			"snapshotVersion is required for operation=act",
		);
	});

	it("requires action for act", async () => {
		await expect(h.invokeTool("web_session", { operation: "act", name: "x", snapshotVersion: 0 })).rejects.toThrow(
			"action is required for operation=act",
		);
	});

	it("propagates a real daemon error (unknown session) through Pi's native error channel", async () => {
		await expect(
			h.invokeTool("web_session", {
				operation: "act",
				name: "does-not-exist",
				snapshotVersion: 0,
				action: "click",
