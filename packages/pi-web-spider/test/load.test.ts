...
import { createExtensionHarness, loadExtensionViaJiti } from "./harness/index.ts";
...
describe("extension load — tryNative:false (Bun binary simulation)", () => {
	...
	it("registers exactly the four tools this extension owns", async () => {
		const factory = await loadExtensionViaJiti(EXTENSION_PATH);
		const h = createExtensionHarness(factory, { cwd: "/tmp" });
		await h.boot();
		...
		expect(h.tools.has("web_category")).toBe(true);
		expect(h.tools.has("web_quotes")).toBe(true);
		await h.shutdown();
	});
});
