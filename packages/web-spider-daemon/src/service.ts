...
const SESSION_ACTIONS = new Set<SessionAction>(["navigate", "click", "hover", "pressKey", "type", "select", "waitFor", "queryText", "readTable", "snapshot", "handleDialog", "downloads", "eval", "screenshot"]);
...
function sessionActInput(input: OperationInput): SessionActInput {
	...
	if (!SESSION_ACTIONS.has(action as SessionAction)) {
		throw new Error('action must be one of "navigate", "click", "hover", "pressKey", "type", "select", "waitFor", "queryText", "readTable", "snapshot", "handleDialog", "downloads", "eval", "screenshot"');
	}
