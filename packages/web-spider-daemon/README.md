...
| `session.close` | Finalizes a named session context-before-browser. Success is destination-idempotent; a failure retains the runtime so the same close can be retried instead of losing cleanup ownership. |
